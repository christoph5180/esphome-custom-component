...
public:
  ...
  void set_custom_fan_mode(const StringRef &fan_mode) {
    ...
    if (fan_mode.empty()) {
      ...
      changed = this->set_custom_fan_mode_(fan_mode);
    }
    ...
    while (available()) {
      ...
      if(len == sizeof(m_get_cmd_resp) && buffer[3] == 0x04) {
        ...
        if (is_valid_xor(buffer, len)) {
          ...
          else if (m_get_cmd_resp.data.fan == 0x00)
            this->set_custom_fan_mode("Automatic");
          else if (m_get_cmd_resp.data.fan == 0x01)
            this->set_custom_fan_mode("1");
          else if (m_get_cmd_resp.data.fan == 0x04)
            this->set_custom_fan_mode("2");
          else if (m_get_cmd_resp.data.fan == 0x02)
            this->set_custom_fan_mode("3");
          else if (m_get_cmd_resp.data.fan == 0x05)
            this->set_custom_fan_mode("4");
